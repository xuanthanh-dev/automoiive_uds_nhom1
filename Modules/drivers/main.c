...
int main(void)
{
  ...
  while (1)
  {
	  ...
	  uint8_t txData[8] =
	      ...
	      if (CAN_IF_Transmit(0x123, txData, 8) == HAL_OK)
	      {
	          printf("TX PASS\r\n");
	      }
	      else
	      ...
	      if (CAN_IF_RxReady == 1)
	      {
	          ...
	          if ((CAN_IF_RxId == 0x123) &&
	              (CAN_IF_RxDlc == 8) &&
	              (CAN_IF_RxData[0] == 0x01) &&
	              (CAN_IF_RxData[1] == 0x02) &&
	              (CAN_IF_RxData[2] == 0x03) &&
	              (CAN_IF_RxData[3] == 0x04) &&
	              (CAN_IF_RxData[4] == 0x05) &&
	              (CAN_IF_RxData[5] == 0x06) &&
	              (CAN_IF_RxData[6] == 0x07) &&
	              (CAN_IF_RxData[7] == 0x08))
	          {
	              printf("RX RESPONSE: PASS\r\n");
	              printf("CAN TX-RX TEST: PASS\r\n");
	          }
	          else
	          {
	              printf("RX RESPONSE: FAIL\r\n");
	              printf("CAN TX-RX TEST: FAIL\r\n");
	          }
	     }
	     else
	     {
	         printf("RX RESPONSE TIMEOUT\r\n");
	     }
