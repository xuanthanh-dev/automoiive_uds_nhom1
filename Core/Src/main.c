...
static const Diag_TestType diagTests[DIAG_TEST_COUNT] = { { "ST-002",
		"Read VIN", { 0x22U, 0xF1U, 0x90U }, 3U },
 ...
 * HAL HANDLES
 *
 * CAN handle is expected to be provided by CAN interface/
 * generated project.
 * ============================================================ */
...
static volatile char diagUartCommand[DIAG_UART_COMMAND_SIZE];
...
static void Diag_HandleCommand(const char *command);
...
static uint8_t Diag_IsResponseExpected(uint8_t testIndex,
		const uint8_t *response, uint16_t length);
...
static void Diag_StoreEngineStatus(const uint8_t *data, uint8_t dlc);
...
int main(void) {
	...
	memset(engineStatusData, 0, sizeof(engineStatusData));

	memset(diagResponse, 0, sizeof(diagResponse));
	...
	memset((void*) diagUartCommand, 0, sizeof(diagUartCommand));
	...
	while (1)
	{
	     ...
	     * PROCESS CAN RX
	     * ======================================================== */
	    Diag_ProcessCan(HAL_GetTick());

	    /* ========================================================
	     * UART COMMAND
	     * ======================================================== */
	    if (HAL_UART_Receive(&huart1, &rxData, 1U, 10U) == HAL_OK)
	    {
	        /* Ignore ENTER */
	        if ((rxData == '\r') || (rxData == '\n'))
	        {
	        ...
	        case '0':
	            ...
	            printf("You selected Engine Status\r\n");
...
static void Diag_PrintMenu(void) {
	...
	printf("9  : ST-010/011 Read active DTC\r\n");

	printf("----------------------------------------\r\n");

	printf("0 : Read Engine Status\r\n");
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
	...
	if (huart->Instance == USART1) {
		...
		else if ((received == '\b') || (received == 127U)) {
			if (diagUartCommandIndex > 0U) {
		 ...
		 * ==================================================== */
			...
			if (diagUartCommandIndex < (DIAG_UART_COMMAND_SIZE - 1U)) {
		...
		(void) HAL_UART_Receive_IT(&huart1, (uint8_t*) &diagUartRxByte, 1U);
	}
}
...
static void Diag_HandleCommand(const char *command) {
	...
	if ((strcmp(command, "A") == 0) || (strcmp(command, "a") == 0)) {
		if (diagWaitingResponse != 0U) {
			printf("[DIAG] Busy - waiting ECU response...\r\n");

			return;
		}

		printf("[DIAG] Run ALL UDS tests\r\n");
	...
	if ((strlen(command) == 1U) && (command[0] >= '1') && (command[0] <= '9')) {
		testIndex = (uint8_t) (command[0] - '1');

		if (testIndex < DIAG_TEST_COUNT) {
			diagRunAll = 0U;

			Diag_StartTest(testIndex);
		}
...
static void Diag_StartTest(uint8_t testIndex) {
	...
	Diag_PrintHex(diagTests[testIndex].request,
			diagTests[testIndex].requestLength);

	printf("\r\n");

	printf("[DIAG] CAN TX 0x%03X: ",
	DIAG_CAN_ID_REQUEST);
	...
	if (Diag_SendCan(
	DIAG_CAN_ID_REQUEST, frame, 8U) != 0U) {
...
static void Diag_ProcessCan(uint32_t now) {
	...
	if ((canId == DIAG_CAN_ID_ENGINE_STATUS) && (dlc == ENGINE_STATUS_DLC)) {
		Diag_StoreEngineStatus(frame, dlc);
...
static void Diag_ReadEngineStatus(void) {
	...
	if (engineStatusAvailable == 0U) {
		...
		printf("[DIAG] Waiting for CAN ID 0x%03X\r\n",
		DIAG_CAN_ID_ENGINE_STATUS);
	...
	memcpy(localData, engineStatusData,
	ENGINE_STATUS_DLC);

	localDlc = engineStatusDlc;
	...
	speed = (uint16_t) (((uint16_t) localData[0] << 8U) | localData[1]);

	rpm = (uint16_t) (((uint16_t) localData[2] << 8U) | localData[3]);

	temperature = localData[4];
...
static void Diag_HandleResponseFrame(const uint8_t *frame, uint8_t dlc) {
	...
	if ((diagWaitingResponse == 0U) || (frame == NULL) || (dlc == 0U)) {
		return;
	}
	...
	if (frameType == 0x00U) {
		payloadLength = (uint16_t) (frame[0] & 0x0FU);

		if ((payloadLength == 0U) || (payloadLength > 7U)
				|| (payloadLength > ((uint16_t) dlc - 1U))) {
			printf("[DIAG] Invalid ISO-TP Single Frame\r\n");

			return;
		}

		memcpy(diagResponse, &frame[1], payloadLength);

		diagResponseLength = payloadLength;
	...
	if ((frameType == 0x10U) && (dlc == 8U)) {
		diagResponseExpectedLength = (uint16_t) (((uint16_t) (frame[0] & 0x0FU)
				<< 8U) | frame[1]);

		if ((diagResponseExpectedLength <= 7U)
				|| (diagResponseExpectedLength > sizeof(diagResponse))) {
			printf("[DIAG] Invalid ISO-TP First Frame\r\n");
		...
		memcpy(diagResponse, &frame[2], 6U);
	...
	if ((frameType == 0x20U) && (diagReceivingConsecutiveFrames != 0U)) {
		sequence = (uint8_t) (frame[0] & 0x0FU);
		...
		remaining =
				(uint16_t) (diagResponseExpectedLength - diagResponseLength);

		copyLength = (remaining > 7U) ? 7U : remaining;

		if (copyLength > ((uint16_t) dlc - 1U)) {
			printf("[DIAG] ISO-TP CF too short\r\n");
		...
		memcpy(&diagResponse[diagResponseLength], &frame[1], copyLength);
		...
		if (diagResponseLength >= diagResponseExpectedLength) {
			diagResponseLength = diagResponseExpectedLength;
	...
	printf("[DIAG] Unknown ISO-TP frame type: 0x%02X\r\n", frame[0]);
}
...
static void Diag_CompleteResponse(void) {
	...
	passed = Diag_IsResponseExpected(diagSelectedTest, diagResponse,
			diagResponseLength);
	...
	if (diagSelectedTest == 8U) {
		...
		if (diagResponseLength >= 10U) {
			if ((diagResponse[3] == 0x01U) && (diagResponse[4] == 0x00U)
					&& (diagResponse[5] == 0x01U)) {
				dtc1Valid = 1U;
			}

			if ((diagResponse[7] == 0x01U) && (diagResponse[8] == 0x00U)
					&& (diagResponse[9] == 0x02U)) {
				dtc2Valid = 1U;
			}
		}
		...
		printf("[DIAG][ST-010] "
				...
				(dtc1Valid != 0U) ? "PASS" : "FAIL");

		printf("[DIAG][ST-011] "
				"LowBattery DTC 0x010002: %s\r\n",
				(dtc2Valid != 0U) ? "PASS" : "FAIL");
	}
...
static uint8_t Diag_IsResponseExpected(uint8_t testIndex,
	...
	if (testIndex == 0U) {
		if ((length >= 3U) && (response[0] == 0x62U) && (response[1] == 0xF1U)
				&& (response[2] == 0x90U)) {
			return 1U;
		}
	...
	if (testIndex == 2U) {
		if ((length == 5U) && (response[0] == 0x62U) && (response[1] == 0x01U)
				&& (response[2] == 0x01U)) {
			return 1U;
		}
	...
	if (testIndex == 4U) {
		if ((length == 3U) && (response[0] == 0x7FU) && (response[1] == 0x22U)
				&& (response[2] == 0x31U)) {
			return 1U;
		}
	...
	if (testIndex == 8U) {
		...
		if ((response[0] != 0x59U) || (response[1] != 0x02U)
				|| (response[2] != 0x09U)) {
			return 0U;
		}
		...
		if ((response[3] != 0x01U) || (response[4] != 0x00U)
				|| (response[5] != 0x01U)) {
			return 0U;
		}
		...
		if ((response[7] != 0x01U) || (response[8] != 0x00U)
				|| (response[9] != 0x02U)) {
			return 0U;
		}
...
static void Diag_SendFlowControl(void) {
	...
	if (Diag_SendCan(
	DIAG_CAN_ID_REQUEST, frame, 8U) == 0U) {
		printf("[DIAG] FlowControl transmit error\r\n");
	}
}
...
static void Diag_CheckTimeout(uint32_t now) {
	if ((diagWaitingResponse != 0U) && ((uint32_t) (now - diagRequestTimeMs) >=
	DIAG_RESPONSE_TIMEOUT_MS)) {
		printf("[DIAG][%s] FAIL - timeout "
...
static uint8_t Diag_SendCan(uint16_t canId, const uint8_t *data, uint8_t dlc) {
	...
	status = CAN_IF_Transmit(canId, (uint8_t*) data, dlc);

	if (status == OK) {
		return 1U;
	}

	printf("[DIAG] CAN_IF_Transmit error: %d\r\n", (int) status);
...
int __io_putchar(int ch) {
	...
	(void) HAL_UART_Transmit(&huart1, &character, 1U,
	HAL_MAX_DELAY);
